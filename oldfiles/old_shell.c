...
void my_execute(char *token,char *cmd,int bgflag){
	pid=fork();
	if (pid==0){
		in=out=out2=0;
		//Child Process
		...
		while(token!=NULL){
			...
			if (out!=1 && in!=1 && strcmp(token,"<")==0){
				...
				int fd2=open(token,O_RDONLY | O_WRONLY|O_APPEND |O_CREAT);
				dup2(fd2,1);
				close(fd2);
				out2=0;
			}
...
int main(int argc,char* argv[],char* envp[]){
	...
	int bgflag=0,index,i,cmd_len,c;
